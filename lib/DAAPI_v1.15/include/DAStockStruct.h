...
struct CStockQryTradeField
{
	...
	TDAStringType		ErrorDescription;					// 错误信息
	TDAStringType		lastFilledNo;						//查询的最后一条用户成交号 update 2020.04.30 ywh
};
// 查询成交返回
struct CStockRspTradeField
{
	...
	TDAStringType		ValidDate;							// 有效日期（1=当日有效, 2=永久有效）
	TDAStringType		AddReduce;							// 开仓还是平仓：1=开仓 2=平仓，3=平今，4=平昨
	TDAStringType		ExchangeTime;						// 交易所返回时间(yyyy-MM-dd hh24:mm:ss) 20151214 add
	TDAStringType		SubClientNo;						// 分账户号
	TDAStringType		FillBrokerList;						// 港股中，成交的时候可以得到成交对手所属的经济商代码
	TDAStringType		ErrorDescription;					// 错误信息
};
// 查询合约请求
